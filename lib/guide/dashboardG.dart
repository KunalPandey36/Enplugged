...
import 'package:enplugged/guide/connectionsG.dart';
...
class DashboardG extends StatefulWidget {
  DashboardG({Key key}) : super(key: key);

  @override
  _DashboardGState createState() => _DashboardGState();
}

class _DashboardGState extends State<DashboardG> {
...
//database section........
  ...
  static final Stream<QuerySnapshot> _investorAll =
      FirebaseFirestore.instance.collection('investorAll').snapshots();
...
//entrepreneur Section .......
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      children: <Widget>[
                        new Text(data['experienceinyear'],
                            style: TextStyle(color: Colors.grey[700])),
                        const SizedBox(width: 8),
                        const SizedBox(width: 8),
                      ],
    ...
    StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        return ListView(
          children: snapshot.data.docs.map((DocumentSnapshot document) {
            Map<String, dynamic> data = document.data() as Map<String, dynamic>;
            String link = "x", type = "x", mail = "x";
            double failpercent = 0, successpercent = 0;
            Color indication;
            String turnoveramount = "x", about = "x";
            link = data['link'];
            turnoveramount = data['totalturnover'];
            successpercent = double.parse(data['successpercentage']);
            failpercent = double.parse(data['failurepercentage']);
            about = data['about'];
            type = data['type'];
            mail = data['email'];
            if (successpercent > failpercent) {
              indication = Colors.green;
            } else {
              indication = Colors.red;
            }
            return Container(
              decoration: BoxDecoration(
                  border: Border.all(color: Colors.white),
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ListTile(
                      ...
                      subtitle: new Text(
                        "$about \n\n Total TurnOver of Firm:Rs$turnoveramount \n\n SuccessPercentage:$successpercent \n\n Company Link:$link",
                        style: TextStyle(color: Colors.black, fontSize: 15),
                      ),
                      onTap: () {
                        postmailid = mail;
    ...
    StreamBuilder(
        ...
        builder: (context, profile) {
          ...
          namepost = name;
          turnover = profile.data["totalstartup"].toString();

          since = profile.data["experienceinyear"].toString();
          sucess = profile.data["successpercentage"].toString();
